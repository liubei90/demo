...
function jian_student_in_depart(mouth, depart) {
  var mouth_depart = depart_tmp[mouth];
  mouth_depart[depart] -= depart_score[depart];
}
...
function get_mini_even(depart) {
  ...
  for (var i = 0; i < must_transfer_department.length; i++) {
    ...
    if (is_even) {
      var mouth_depart = depart_tmp[i];
      if (mouth_depart[depart] < min) {
        min = mouth_depart[depart];
        min_index = i;
      }
    }
  }
